...
class Chapter3ProjectStructureTests(TestCase):
    ...
    def test_project_created(self):
        ...
        directory_exists = os.path.isdir(
            os.path.join(self.project_base_dir, "tango_with_django_project")
        )
    ...
    def test_rango_app_created(self):
        ...
        is_python_package = os.path.isfile(
            os.path.join(self.rango_app_dir, "__init__.py")
        )
        views_module_exists = os.path.isfile(
            os.path.join(self.rango_app_dir, "views.py")
        )
...
class Chapter3IndexPageTests(TestCase):
    ...
    def test_view_exists(self):
        ...
        self.assertTrue(
            name_exists,
            f"{FAILURE_HEADER}The index() view for rango does not exist.{FAILURE_FOOTER}",
        )
        self.assertTrue(
            is_callable,
    ...
    def test_for_about_hyperlink(self):
        ...
        single_quotes_check = (
            "<a href='/rango/about/'>About</a>" in response.content.decode()
            or "<a href='/rango/about'>About</a>" in response.content.decode()
        )
        double_quotes_check = (
            '<a href="/rango/about/">About</a>' in response.content.decode()
            or '<a href="/rango/about">About</a>' in response.content.decode()
        ...
        self.assertTrue(
            single_quotes_check or double_quotes_check,
